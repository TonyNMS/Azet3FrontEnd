...
import CarbonPlotter from "./Optimisation/CarbonPlotter";
import "./Styling/Co2Optimisation.css"
...
const CO2Optimisation =()=>{
  ...
  const renderPriotiySelectOptions =()=>{
    return PRIORITY_OPTIONS.map((item, index)=>(
        <option key={`carbon-emi-SA-${index}`} value = {item.value}>{item.name}</option>
    )           
  )}
  const renderTrueNFalse=()=>{
    return [
        <option key={`carbon-emi-SA-tf-1`} value = "true">Yes</option>,
        <option key={`carbon-emi-SA-tf-2`} value = "false">No</option>
    ]
  }
...
export default CO2Optimisation;